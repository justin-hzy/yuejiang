...
public class ThSaleCheckAction extends BaseBean implements Action {
    @Override
    public String execute(RequestInfo requestInfo) {
        ...
        if (detailDatas1.size() > 0) {
            ...
            for (Map<String, String> detailData : detailDatas1) {
                String khddh = detailData.get("erp_order");
            ...
            for (String orderNo : hashSet){
                String sql = "select * from formtable_main_347 main where erp_order = ?";
        ...
        if(orderNos.size()>0){
            ...
            String updateSql = "update formtable_main_350 set cause = ? , isRpt = ? where requestid = ?";
            RecordSet dtRs = new RecordSet();
